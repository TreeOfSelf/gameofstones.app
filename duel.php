...
// Check if 'enemyid' is set in the POST request
if (isset($_POST['enemyid'])) {
    $enemy_id = intval($_POST['enemyid']);
} else {
    // Handle the case where 'enemyid' is missing by redirecting
    $redirect = 1;
    $rmsg = 'Error: No opponent specified for duel.';
    $enemy_id = 0; // Set to 0 just to avoid potential later uninitialized variable warnings, though redirect should prevent use.
}
...
if (($find_battle[$enemy_id] > time() - intval(30*(100+$dw)/100)) && $debug_mode!=true) // DUEL FIX: 0 -> 300 
{ 
  $redirect=1;
...
if ($time_dif >= 0 && $char['battlestoday'] < $battlelimit) {
  if(!$debug_mode) $char['battlestoday'] += $multiply;
...
if ($debug_mode != true)  $char['stamina']-=$stamina1*$multiply;
if ($char['stamina']<0) $char['stamina']=0;
if ($debug_mode != true)  $char2['stamina']-=$stamina2*$multiply;  
