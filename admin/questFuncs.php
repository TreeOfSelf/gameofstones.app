...
function check_inventory_items ($item, $inventory, $type=-1, $item_base='')
{
  ...
  {
    $found = 1; // Default to found, checks below will set to 0 if mismatch
    if ($inv[$i]['istatus']<=0) // Check usable status
    {
      if ($type != -1) { // "Items" quest: Check only by type
          if ($type < 12) { // Regular item type
              if ($inv[$i]['type'] != $type) {
                  $found = 0;
              }
          } elseif ($type == 12) { // Ter'angreal (type 12 or 13)
              if ($inv[$i]['type'] != 12 && $inv[$i]['type'] != 13) {
                  $found = 0;
              }
          }
          // No Base/Prefix/Suffix check needed for generic type match
      } else { // "Request" quest: Check by specific Base/Prefix/Suffix
          // Base Check
          if ($itm[0] != '*' && strtolower($inv[$i]['base']) != strtolower($itm[0])) {
              $found = 0;
          }
          // Prefix Check
          if ($found && $itm[1] != '*' && strtolower($inv[$i]['prefix']) != strtolower($itm[1])) {
              $found = 0;
          }
          // Suffix Check
          if ($found && $itm[2] != '*' && strtolower($inv[$i]['suffix']) != strtolower($itm[2])) {
              $found = 0;
          }
      }
    }
    else $found=0; // item has non-zero istatus
