...
$officer = mysqli_real_escape_string($db,$_POST['officer']);
$nindex = mysqli_real_escape_string($db,$_POST['index']);
$add = mysqli_real_escape_string($db,$_POST['add']);
$sell = mysqli_real_escape_string($db,$_POST['sell']);
$abandon = mysqli_real_escape_string($db,$_POST['abandon']);
$restock = mysqli_real_escape_string($db,$_POST['restock']);
$tab = mysqli_real_escape_string($db,$_GET['tab']);
$transGood = mysqli_real_escape_string($db,$_POST['transGood']);
$transFrom = mysqli_real_escape_string($db,$_POST['transFrom']);
$transAmount = mysqli_real_escape_string($db,$_POST['transAmount']);
...
if ($add != "")
{
  ...
  if ($society['bank'] >= $cost)
  {
    ...
    {
      $society['bank'] -= $cost;
      mysqli_query($db,"UPDATE Soc SET bank='".$society['bank']."' WHERE name='$soc_name'");  
      ...
      mysqli_query($db,"UPDATE Locations SET shoplvls='$shoplvls_str' WHERE name='$loc_query'");
...
if ($sell != "")
{
  ...
  if ($shoplvls[$nindex][$society['id']]>=$sell)
  {
    ...
    mysqli_query($db,"UPDATE Soc SET bank='".$society['bank']."', area_rep='".$area_reps."' WHERE name='$soc_name'");
  ...
  { 
    if ($office_reqs[count($offices)] <= $society['score'])
    {
      ...
      if ($cost <= $society['bank'])
      {
        ...
        mysqli_query($db,"UPDATE Soc SET offices='$offices_ser', bank='".$society['bank']."', area_rep='".$area_reps."' WHERE name='$soc_name'");
...
if ($officer)
{
  ...
  $offices_ser = serialize($offices);
  $result = mysqli_query($db,"UPDATE Soc SET offices='$offices_ser' WHERE name='$soc_name'");
...
if ($transFrom)
{
  ...
  if ($transFrom == 99)
  {
    ...
    if ($cgoods[$transGood] >= $transAmount)
    {
      ...
      $result = mysqli_query($db,"UPDATE Soc SET goods='$scgoods' WHERE name='$soc_name'");
      $result2 = mysqli_query($db,"UPDATE Locations SET ".$gname."='".$location[$gname]."' WHERE name='$loc_query'");
    ...
    if ($city['id'])
    {
      if ($city[$gname] > $transAmount)
      {
        ...
        if ($maxTransfer >= $transAmount)
        {
          ...
          $location[$gname] += $transAmount;
          $result2 = mysqli_query($db,"UPDATE Locations SET ".$gname."='".$city[$gname]."' WHERE name='$city[name]'");
          $result2 = mysqli_query($db,"UPDATE Locations SET ".$gname."='".$location[$gname]."' WHERE name='$loc_query'");
    ...
    ?>    
      <div class='row'>
        <div class='col-sm-4 col-sm-push-4'>
          ...
          <form action="clanoffice.php" method="post">
            <div class="form-group form-group-sm">
              ...
              <?php
                ...
                $result = mysqli_query($db,"SELECT id, name, lastname FROM Users WHERE society='$soc_name' ORDER BY lastname, name");
          ...
          <form onsubmit="return confirm('Do you really want to abandon this office?');" action="clanoffice.php" method="post">
            <div class="form-group form-group-sm">
