...
$show=intval(mysqli_real_escape_string($db,$_GET['show']));
$sell=intval(mysqli_real_escape_string($db,$_GET['sell']));
$proup=mysqli_real_escape_string($db,$_POST['proup']);
...
mysqli_query($db,"UPDATE Users SET newprof=0 WHERE id=$id");
...
if ($proup != '' && $proup != '-1')
{
  if ($char['propoints']>0)
  {
    if ( $jobs[$proup] < 9)
    {
      ...
      mysqli_query($db,"UPDATE Users SET jobs='".$newjobs."', propoints=".$char['propoints']." WHERE id='".$char['id']."'");
      mysqli_query($db,"UPDATE Users_stats SET prof_pts_used='".$stats['prof_pts_used']."', highest_prof=".$stats['highest_prof'].", num_profs='".$stats['num_profs']."' WHERE id='".$char['id']."'");
    }
    else
