  ...
  if ($good && $q > 0) // All is good
  {
    if ($scoreup) // Did this quest add Ji?
    {
      ...
      if ($quest['type'] == $quest_type_num["Escort"])
      {
        ...
        if (!empty($qloc2['id'])) {
          $result = mysqli_query($db,"UPDATE Users_stats SET loc_ji".$qloc2['id']."='".$ustats['loc_ji'.$qloc2['id']]."' WHERE id='$id'");
        }
      }
      if ($quest['align'] !=0)
      {
...
elseif ($submitted != '') // Non-Item Quests
{
  ...
  if ($percomp >= 100 || $debug_mode == true)  // Are you done
  {  
    if (($quest['type'] == $quest_type_num["Items"] && $goals[2] == $char['location']) || $quest['type'] != $quest_type_num["Items"]) // Right place to submit?
    {
      if ($myquests[$submitted][0] != 2) // Already done?
      {
        ...
        {
          error_log("Item reward: " . $qreward[1]);
          $itmlist=[];
          $listsize=0;
        ...
        elseif ($qreward[0] == "LI") // Leveled item reward
        {
          ...
          { 
            ...
            $itemName = ucwords($prefix);
            if ($itemName) $itemName .= " ";
            $itemName .= ucwords($base);
            if ($suffix) $itemName .= " ".str_replace("Of","of",ucwords($suffix));
            $message = "Quest submitted. You earned a ".$itemName.".";
          } 
          else
          {  
            $good = 0;
        ...
        if ($good) // Are we good?
        {
          if ($scoreup) // Quest adds Ji
          {
            ...
            if ($quest['type'] == $quest_type_num["Escort"])
            {
              ...
              if ($qloc2['next_wins'] <= $area_score[$qloc2['id']])
              {
                ...
                mysqli_query($db,"UPDATE Locations SET ruler='".$society['name']."' WHERE id='$qloc2[id]'");
              }
            }
            $area_score[$qloc['id']] = $area_score[$qloc['id']]+$scoreup;
            $area_score[$qloc['id']] = number_format($area_score[$qloc['id']],2,'.','');
            if ($qloc['next_wins'] <= $area_score[$qloc['id']])
            {
              ...
              mysqli_query($db,"UPDATE Locations SET ruler='".$society['name']."' WHERE id='$qloc[id]'");
            }
            $a_s_str = serialize($area_score);
          ...
          // Set a default message if none was set (money reward or regular quests)
          if (!strstr($message, "Quest submitted") && !strstr($message, "Item") && $good) {
            ...
            <form name="questForm" action="myquests.php" method="post">
              ...
              ?>
                ...
                ?>
                  ...
                  <td align='center'>
                <?php
                  if ($percomp >= 100 || $debug_mode == true)
                  {
