...
$time = time();
{
	//POSSIBLE FUCKUP $buy>0
  if ($buy>0 && iname_list($buy-1,$itmlist) == $_GET['name'] && $buy>0)
  {
    $buy--;
    ...
    if ($char['gold'] >= $worth)
    {
      ...
      if ($itmsize < $inv_max)
      {
        ...
        mysqli_query($db,"UPDATE Users SET gold='$char[gold]', lastbuy='$time' WHERE id=$id");
        ...
        if ($shoplvls[$buy][$sellclan]>0)
        {
          ...
          mysqli_query($db,"UPDATE Locations SET shoplvls='$shoplvls_str' WHERE id='".$location['id']."'");
          $itmlist[$buy][4]--;
        }
        
        $message = mysqli_real_escape_string($db,$_GET['name'])." purchased for ".displayGold($worth);
      }
      else 
      {
            ...
            if ($displayType[$t])
            {
              ...
              for ($i = 0; $i < 13; ++$i)
              {
                if ($displayType[$t][$i])
                {
                  ...
                  $shopinfo[$t][$i] .= "<button type='button' class='btn btn-info btn-sm btn-wrap' data-html='true' data-toggle='popover' data-placement='top' data-content='".htmlspecialchars($shopstats[$t][$i], ENT_QUOTES, 'UTF-8')."'>Info</button>";                  
                  if ($worth <= $char['gold']) 
                  {
    ...
    ?>
          ...
          <?php 
            ...
            for ($x=0; $x < $foundnum; $x++)
            {
              ...
              $shopinfo .= "<button type='button' class='btn btn-info btn-sm btn-wrap' data-html='true' data-toggle='popover' data-placement='top' data-content='".htmlspecialchars($shopstats, ENT_QUOTES, 'UTF-8')."'>Info</button>";                  
              if ($worth <= $char['gold']) 
              {
