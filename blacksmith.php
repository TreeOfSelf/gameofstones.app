...
$pts_tot = 0;
if (!empty($a)) $pts_tot += (lvl_req($estats[1], getTypeMod($skills,$a['type'])));
if (!empty($b)) $pts_tot += (lvl_req($estats[2], getTypeMod($skills,$b['type'])));
if (!empty($c)) $pts_tot += (lvl_req($estats[3], getTypeMod($skills,$c['type'])));
if (!empty($d)) $pts_tot += (lvl_req($estats[4], getTypeMod($skills,$d['type'])));
if (!empty($e)) $pts_tot += (lvl_req($estats[5], getTypeMod($skills,$e['type'])));
  ...
  if ($total <= $char['gold'])
  {
    if (!$overeq)
    {
      if ($myPoints >= 0)
      {
        for ($x=0; $x<$q; $x++)
        {
          $result = mysqli_query($db,"UPDATE Items SET cond='100' WHERE id='".$itemp[$x]['id']."'");
        ...
        $query = mysqli_query($db,"UPDATE Locations SET bank='$location[bank]' WHERE name='$loc_query'");
        $query = mysqli_query($db,"UPDATE Users SET gold='$char[gold]', used_pts='$char[used_pts]' WHERE id=$id");
        $query = mysqli_query($db,"UPDATE Users_stats SET items_repaired='".$ustats['items_repaired']."', spent_repair='".$ustats['spent_repair']."' WHERE id=$id");
      }
      else 
      { 
