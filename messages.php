...
$resultnumb = mysqli_real_escape_string($db,$_GET['resultnumb']);
if ($resultnumb == "") $resultnumb = 0;
$resultnumbout = mysqli_real_escape_string($db,$_GET['resultnumbout']);
if ($resultnumbout == "") $resultnumbout = 0;
$resultnumblog = mysqli_real_escape_string($db,$_GET['resultnumblog']);
if ($resultnumblog == "") $resultnumblog = 0;

// update message check
mysqli_query($db,"UPDATE Users SET msgcheck='".time()."' WHERE id=$id");
...
$tid = mysqli_real_escape_string($db,$_GET['id']);
      ...
      if ($char['gold']-$data['2']>=0)
      {
        ...
        if ($qitem['owner'] == $data['0'])
        {
          if ($qitem['society'] == 0)
          {
            ...
            {
              $listsize=mysqli_num_rows(mysqli_query($db,"SELECT * FROM Items WHERE owner='$id' AND type<15"));

              if ($listsize<$inv_max)
              {
                $qitem['owner']=$id;
                $result = mysqli_query($db,"UPDATE Items SET owner='$id', last_moved='".time()."' WHERE id='".$qitem['id']."'");
                $tnote['special']="";
                $tnote['body']=preg_replace('/\[.*\]/',"['trade completed']",$tnote['body']);
                $char['gold']-=$data['2'];
                mysqli_query($db,"UPDATE Notes SET body='".$tnote['body']."', special='".$tnote['special']."' WHERE id=".$tnote['id']);
                mysqli_query($db,"UPDATE Users SET gold='".$char['gold']."' WHERE id=".$char['id']);
                mysqli_query($db,"UPDATE Users SET gold=gold+".$data[2]." WHERE id=".$data[0]);
...
if ($_GET['trade'] && $_REQUEST['item'])
{
  ...
  if ($target)
  {
    ...
    {
      $cid = $target['id'];
	  if(is_numeric($tradeval=$_REQUEST['gold']) && 
	  is_numeric($tradeval=$_REQUEST['copper']) &&
	  is_numeric($tradeval=$_REQUEST['silver'])){
...
mysqli_query($db,"UPDATE Users SET newmsg=0 WHERE id=$id");
